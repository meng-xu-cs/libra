...
use crate::{
    ...
    workflow::{prepare_with_override, prove, WorkflowOptions},
};
...
fn remove_first_aborts_if_and_prove_since(
    ...
    let (env, targets) = prepare_with_override(options, model_pipeline, spec_override)?;
...
fn remove_redundant_aborts_ifs_since(
    ...
    match remove_first_aborts_if_and_prove_since(options, model_pipeline, fun_id, &spec, pos)? {
        None => {
        ...
        Some((new_spec, true)) => {
            ...
            remove_redundant_aborts_ifs_since(options, model_pipeline, fun_id, new_spec, pos)
        }
        Some((_, false)) => {
            // removing one aborts_if makes the proving failed
            remove_redundant_aborts_ifs_since(options, model_pipeline, fun_id, spec, pos + 1)
        }
    }
}
