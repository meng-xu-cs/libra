...
impl PerFunctionRelevance {
    ...
    fn analyze(target: &FunctionTarget, targets: &FunctionTargetsHolder) -> Self {
        ...
        //
        // NOTE: why do we use the `InvariantRelevance::direct_accessed` set instead of other sets?
        //
        // - The reason we choose `direct_accessed` over `accessed` is that we don't want to
        //   assume invariants that are applicable to callees only and not applicable to the caller.
        //
        //   Based on how `InvariantRelevance` is derived, recall that if a suspendable invariant is
        //   delegated to the caller, that invariant will appear in the `direct_accessed` set on the
        //   caller side.
        //
        //   It does not hurt (in terms of soundness or completeness of the proofs) to assume extra
        //   invariants in the `accessed` set, but these assumptions won't be actually used in
        //   proofs of any asserts. Therefore, they do not need to be assumed.
        //
        ...
        let entrypoint_assumptions = Self::calculate_invariant_relevance(
            env,
            mem_analysis.accessed.direct.iter(),
