...
impl TypeUnificationAdapter {
    /// Initialize the context for the type unifier. If `treat_lhs_type_param_as_var_after_index`
    /// is set to P, then any type parameter on the LHS with index < P will be treated as concrete
    /// types and only type parameters on the LHS with index >= P are treated as variables and
    /// participate in the type unification process. The same logic applies to the RHS type
    /// parameters via `treat_rhs_type_param_as_var_after_index`.
    fn new<'a, I>(
        lhs_types: I,
        rhs_types: I,
...
impl TypeInstantiationDerivation {
    ...
    /// But in anyway, this type parameter no longer participate in type unification anymore.
    ...
    ///
    /// The algorithm is progressive. For a list of parameters with arity `params_arity`, it
    /// - first finds all possible instantiation for parameter at index 0 (`inst_0`) and,'
    /// - for each instantiation in `inst_0`,
    ///   - refines LHS or RHS types and
    ///   - finds all possible instantiations for parameter at index 1...
    /// The process continues until all type parameters are analyzed (i.e., reaching the
    /// `params_arity`).
    ///
    /// If `refine_lhs` is set, refine the `lhs_types` after each round; same for `refine_rhs`.
    /// If `mark_irrelevant_param_as_error` is set, type parameters that do not have any valid
